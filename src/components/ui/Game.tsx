...
export default function Game() {
  ...
  useEffect(() => {
    ...
    const handleRequestLoadingTrueEvent = () => {
      console.log('request loading true');
      setIsRequestLoading(true);
    };

    const handleRequestLoadingFalseEvent = () => {
      console.log('request loading false');
      setIsRequestLoading(false);
    };

    addEventListener('data_loaded', handleDataLoadedEvent);
    addEventListener('respawn', handleRespawnEvent);
    addEventListener('request_loading_true', handleRequestLoadingTrueEvent);
    addEventListener('request_loading_false', handleRequestLoadingFalseEvent);
    return () => {
      removeEventListener('data_loaded', handleDataLoadedEvent);
      removeEventListener('respawn', handleRespawnEvent);
      removeEventListener(
        'request_loading_true',
        handleRequestLoadingTrueEvent,
      );
      removeEventListener(
        'request_loading_false',
        handleRequestLoadingFalseEvent,
      );
    };
  ...
  return (
    <div className="relative flex justify-center items-center">
      ...
      {isRequestLoading && (
        <div className="absolute top-0 left-0 w-[800px] h-[500px]  rounded-[20px] bg-zinc-950/20 flex justify-center items-center backdrop-blur-xs z-10">
          <svg
            className="animate-spin h-8 w-8 text-cyan-50"
